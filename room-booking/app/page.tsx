...
// IMPORTING YOUR REAL BACKEND LOGIC!
...
export default function Home() {
  ...
  const [messages, setMessages] = useState([
    { role: "system", text: "Hello! I am your booking assistant. Please enter your name at the top right, then click an empty slot to begin." }
  ]);
  ...
  // FETCH REAL BOOKINGS WHENEVER THE ROOM CHANGES
  useEffect(() => {
    async function loadBookings() {
      const result = await getBookings(selectedRoom);
      if (result.success) {
        setRealBookings(result.data);
      }
    }
    loadBookings();
  }, [selectedRoom]);
  ...
  const formatIsoString = (date: number, timeStr: string) => {
    // Convert "02:00 PM" to 24hr "14:00:00"
    const [time, modifier] = timeStr.split(" ");
    let [hours, minutes] = time.split(":");
    if (hours === "12") hours = "00";
    if (modifier === "PM") hours = (parseInt(hours, 10) + 12).toString();
    
    // Formatting as May 2026 (Pad single digits with zero)
  ...
  const handleSlotClick = (date: number, dayName: string, time: string, isBooked: boolean) => {
    ...
    setMessages(prev => [
      ...prev,
      { role: "system", text: `I have penciled in ${time} on ${dayName}, May ${date} for ${selectedRoom}. Type 'confirm' to permanently lock this room.` }
    ]);
  };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // THE REAL MAGIC: Checking if they typed "confirm"
    if (input.toLowerCase() === "confirm" && selectedSlot) {
      ...
      // Calculate end time (assuming 1 hour slots for the UI)
      ...
      if (result.success) {
        ...
        if (refresh.success) setRealBookings(refresh.data);
        setSelectedSlot(null); // Clear the draft
      } else {
        setMessages(prev => [...prev, { role: "system", text: `❌ Blocked: ${result.error}` }]);
      }
      setIsLoading(false);
    } else if (input.toLowerCase() === "confirm" && !selectedSlot) {
      setMessages(prev => [...prev, { role: "system", text: "You need to click an empty time slot on the calendar first!" }]);
    } else {
      setMessages(prev => [...prev, { role: "system", text: "I only understand the word 'confirm' right now. Please select a slot and type 'confirm'." }]);
    }
  };
                  ...
                  {weekDays.map((day) => (
                    <div key={`col-${day.date}`} className="col-span-1 border-r border-gray-100">
                      {hours.map((hour) => {
                        ...
                        // Check REAL database for existing bookings in this exact slot
        ...
        <div className="p-4 bg-white border-t border-gray-100">
          <form onSubmit={handleSendMessage} className="max-w-6xl mx-auto w-full flex gap-2 sm:gap-3">
            <input
              ...
              placeholder="Type 'confirm' to book..."
